#!/usr/bin/env python3
'''
This script generates a CSV file of all CommunicationRequest resources in the system.
To run: 
  docker-compose run --volume=${PWD}/utils:/opt/utils femr \
    python3 /opt/utils/communicationrequest_report.py
...
def extract_type(cr):
    for category in cr.get('category', []):
...
def main():
    url = f"{base_url}/CommunicationRequest?_sort=recipient"
    patient_cache = {}

    with open('CommunicationRequestReport.csv', 'w', newline='') as file:
        ...
        while url:
            ...
            for entry in data.get('entry', []):
                ...
                if type in ['isacc-comment', 'isacc-non-sms-message']:
                    recipient_reference = cr.get('subject', {}).get('reference')
                else:
                    recipient_reference = cr.get('recipient', [{}])[0].get('reference')
                if recipient_reference == None:
                    recipient_reference = cr.get('sender', {}).get('reference')
                isacc_id = get_patient_isacc_id(recipient_reference, patient_cache)
                content = cr.get('payload', [{}])[0].get('contentString')
                if content:
                    content = content.replace('\n', '\\n')
                sender = cr.get('sender', {}).get('reference')
                if sender == None and type == 'isacc-auto-sent-message':
                    sender = 'system'
                note = cr.get('note', [{}])[0].get('text')
                if note:
                    note = note.replace('\n', '\\n')

                writer.writerow([cr_id, datetime, recipient_reference, isacc_id, type, status, content, sender, note])
